...
describe("Ubuntu installer packaging contract", () => {
  it("builds only from the validated staged source directory", () => {
    ...
    expect(installer).toContain('configured provider routing without hidden Gemini editorial calls, source-context retrieval with canonical source reconciliation, terminal workflow states with failure diagnostics');
