...
Managed PostgreSQL always uses `postgres@db:5432/editorial_db`. Older arbitrary usernames are replaced and the private role password is reconciled with root-only configuration through the container-local administrator socket. Authentication checks and container health checks then connect through the database container's Docker bridge hostname, following the same `scram-sha-256` host rule as the application. This avoids false success from a trusted `127.0.0.1` rule on an older volume. PostgreSQL values are passed explicitly to schema-bootstrap and application containers rather than relying only on `env_file` parsing. Matching per-run salted fingerprints in the installer and bootstrap logs expose any remaining handoff mismatch without disclosing the password. This repairs missing-role and password-propagation failures without deleting the PostgreSQL volume.

Public HTTPS is domain-based. Before installation, create a DNS `A` record for a hostname you control and point it to the server. Access the URL printed by `status` or `verify`; a raw server IP is not the configured certificate identity and can return `SSL_ERROR_INTERNAL_ERROR_ALERT`. Placeholder and reserved example names are rejected. Deployment requires DNS resolution, validates and reloads Caddy explicitly, and verifies the trusted HTTPS health endpoint before recording success. Correct an older stored placeholder by rerunning install with `--domain` followed by the real hostname you control.
