...
Public HTTPS is domain-based. Access the URL printed by `status` or `verify`; a raw server IP is not the configured certificate identity and can return `SSL_ERROR_INTERNAL_ERROR_ALERT`. Deployment validates and reloads Caddy explicitly so an already-running proxy cannot retain a Caddyfile from an earlier interrupted attempt.
