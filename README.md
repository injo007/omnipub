...
The installer now pins the managed identity to `postgres@db:5432/editorial_db`, synchronizes the private `postgres` role password with the root-only installer configuration before schema bootstrap, and verifies authentication through the database container's Docker bridge address. This bridge-path check is important because an older volume can trust `127.0.0.1` while still rejecting the application's `scram-sha-256` connection, making a loopback test report a false success. PostgreSQL variables are passed explicitly to the bootstrap and application containers so Compose `env_file` parsing cannot produce a different runtime password. The installer and one-shot bootstrap also log matching, per-run salted credential fingerprints and byte counts for diagnosis without logging the password or a reusable password hash. It does not delete the volume or application data. Rerun the normal installation command and leave the PostgreSQL password blank to reuse the stored value.
